...
const windows1252ByteByCodePoint = new Map([
  [0x20ac, 0x80],
  [0x201a, 0x82],
  [0x0192, 0x83],
  [0x201e, 0x84],
...
function looksLikeMojibake(value) {
  return mojibakeSignals.some((pattern) => pattern.test(value));
}
...
function toWindows1252Bytes(value) {
  ...
  for (const character of value) {
    ...
    const byte = windows1252ByteByCodePoint.get(codePoint);
...
function repairMojibakeString(value) {
  ...
  for (let attempt = 0; attempt < 3 && looksLikeMojibake(repaired); attempt += 1) {
    ...
    if (!decoded || decoded === repaired || (!repaired.includes("\uFFFD") && decoded.includes("\uFFFD"))) {
      break;
    }
...
function repairCmsValue(value) {
  if (typeof value === "string") {
    return repairMojibakeString(value);
  }

  if (Array.isArray(value)) {
    return value.map((item) => repairCmsValue(item));
  }

  if (value && typeof value === "object") {
    return Object.fromEntries(
      Object.entries(value).map(([key, item]) => [key, repairCmsValue(item)]),
    );
  }
...
async function fetchItems(resource, config, baseUrl, token) {
  ...
  return normalizePayload(payload, config, resource)
    ...
    .map((item) => normalizeItem(resource, repairCmsValue(item)));
}
